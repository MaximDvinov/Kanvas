    ...
    return engine {
        scene("space", setCurrent = true) {
            camera {
                ...
                keyboardDefaults()
                gestureDefaults(
                    enabled = { interactionMode() == InteractionMode.Camera },
                )
            }
                ...
                runtimeScene?.spawn(
                    Entity("drag-preview").apply {
                        addComponent(RenderComponent(zIndex = 200) {
                            ...
                            if (interactionMode() == InteractionMode.Add && start != null && current != null) {
            ...
            onInput { event ->
                when (event) {
                    ...
                    is Drag -> {
                        if (interactionMode() == InteractionMode.Camera) return@onInput
                        activeDragCurrent = event.position
